...
void send_mes(int fd, int type, PACK *recv_pack, char *mes)
{
    ...
    pack_send.type = type;
    strcpy(pack_send.data.send_user, pack_send.data.send_user);
...
void look_fri(PACK *recv_pack)
{
    ...
    int statu_s;
    memset(need, 0, sizeof(need));
    sprintf(need, "select *from friends where friend_user = '%s' or user='%s'", recv_pack->data.send_user,recv_pack->data.send_user);
    printf("%s\n",need);
...
void add_fri(PACK *recv_pack)
{
    ...
    while(q)
    {
        if((strcmp(q->user, recv_pack->data.recv_user) == 0 && strcmp(q->friend_user, recv_pack->data.send_user) == 0) || (strcmp(q->user, recv_pack->data.send_user) == 0 && strcmp(q->friend_user, recv_pack->data.recv_user) == 0))
        {
            flag_3 = 1;
            break;
        }
...
void add_grp(PACK *recv_pack)
{
    ...
    {   
        ...
        memset(need, 0, strlen(need));
        sprintf(need, "insert into friends values('%s', '%s', %d)", recv_pack->data.send_user, recv_pack->data.mes, GRP);
        mysql_real_query(&mysql, need, strlen(need));
        send_mes(fd, flag, recv_pack, ch);
        return;
    }
