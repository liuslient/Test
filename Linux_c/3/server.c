...
void send_pack(int fd, PACK *recv_pack, char *ch)
{
    PACK pack_send;
    memcpy(&pack_send, recv_pack, sizeof(PACK));
    printf("%s\t%s\n", pack_send.data.recv_user, pack_send.data.send_user);
    strcpy(pack_send.data.recv_user, pack_send.data.send_user);
...
void registe(PACK *recv_pack_t)
{
    ...
    if(flag == 0)
    {
        ...
        Insert(pNew);
        memset(need, 0, sizeof(need));
	    sprintf(need, "insert into user_data values('%s','%s',%d,%d)",  recv_pack->data.send_user, recv_pack->data.mes, 0, recv_pack->data.send_fd);
        printf("%s\n",need);
    	mysql_query(&mysql, need);
        ch[0] = '1';
    }
...
void cre_grp(PACK *recv_pack_t)
{
    ...
    else
    {
        ...
        memset(query_str, 0, strlen(query_str));
        sprintf(query_str, "insert into friends values('%s', '%s', %d)", recv_pack->data.send_user, recv_pack->data.mes, GRP_OWN);
        mysql_real_query(&mysql, query_str, strlen(query_str));
    }
...
void add_grp(PACK *recv_pack_t)
{
    ...
    {   
        ...
        memset(query_str, 0, strlen(query_str));
        sprintf(query_str, "insert into relationinfo values('%s', '%s', %d)", recv_pack->data.recv_user, recv_pack->data.send_user, GRP);
        mysql_real_query(&mysql, query_str, strlen(query_str));
        send_more(fd, flag, recv_pack, ch);
        return;
    }
...
void out_grp(PACK *recv_pack_t)
{
    ...
    else
    {
        ...
        memset(query_str, 0, strlen(query_str));
        sprintf(query_str, "delete from friends where user='%s' and friend_user='%s'", recv_pack->data.send_user, recv_pack->data.mes);
        mysql_real_query(&mysql, query_str, strlen(query_str));
    }
    send_more(fd, flag, recv_pack, ch);
}
...
void del_grp(PACK *recv_pack_t)
{
    ...
    else if(flag_3 == 1 && flag_3_3 == 1)
    {
        ...
        memset(query_str, 0, strlen(query_str));
        sprintf(query_str, "delete from friends where friend_user='%s'", recv_pack->data.mes);
        mysql_real_query(&mysql, query_str, strlen(query_str));
    }
...
void set_grp_adm(PACK *recv_pack_t)
{
    ...
    if(flag_3 == 1 && flag_3_3 == 1 && flag_3_3_3 == 1)
    {
        ...
        memset(query_str, 0, strlen(query_str));
        sprintf(query_str, "update friends set status=%d where user='%s' and friend_user='%s'", GRP_ADM, recv_pack->data.mes, recv_pack->data.recv_user);
        mysql_real_query(&mysql, query_str, strlen(query_str));
    }
...
void kick_grp(PACK *recv_pack_t)
{
    ...
    if(flag_3 == 1 && flag_3_3 == 1 && flag_3_3_3 == 1 && flag_3_3_3_3 == 1)
    {
        ...
        memset(query_str, 0, strlen(query_str));
        sprintf(query_str, "delete from friends where user='%s' and friend_user='%s'", recv_pack->data.mes, recv_pack->data.recv_user);
        mysql_real_query(&mysql, query_str, strlen(query_str));
    }
...
void chat_many(PACK *recv_pack_t)
{
    ...
    else
    {
        ...
        {
            memset(query_str, 0, strlen(query_str));
            sprintf(query_str, "select * from chat_messages where recv_user='%s'", recv_pack->data.recv_user);
            mysql_real_query(&mysql, query_str, strlen(query_str));
            res = mysql_store_result(&mysql);
            rows = mysql_num_rows(res);
            fields = mysql_num_fields(res);
            ...
            while(q)
            {
                if(strcmp(q->user, recv_pack->data.send_user) != 0 && strcmp(q->friend_user, recv_pack->data.recv_user) == 0 && (q->realtion >= GRP))
                {
                    t = pHead;
                    while(t)
                    {
        ...
        else
        {
            ...
            Insert_RC(pNew);
            memset(query_str, 0, strlen(query_str));
            sprintf(query_str, "insert into chat_messges values('%s', '%s', '%s')", recv_pack->data.send_user, recv_pack->data.recv_user, recv_pack->data.mes);
            mysql_real_query(&mysql, query_str, strlen(query_str));
...
void check_mes_fri(PACK *recv_pack_t)
{
    ...
    while(q)
    {
        if(((strcmp(q->user, recv_pack->data.send_user) == 0 && strcmp(q->friend_user, recv_pack->data.mes) == 0) || (strcmp(q->friend_user, recv_pack->data.send_user) == 0 && strcmp(q->user, recv_pack->data.mes) == 0)) && (q->realtion == FRIEND)) 
        {
            flag_2 = 1;
            break;
        }
...
void check_mes_grp(PACK *recv_pack_t)
{
    ...
    while(q)
    {
        if(strcmp(q->user, recv_pack->data.send_user) == 0 && strcmp(q->friend_user, recv_pack->data.mes) == 0 && (q->realtion >= GRP)) 
        {
            flag_2 = 1;
            break;
        }
